...
export function createFieldValueSchema(
  ...
  switch (type) {
    case 'number':
      schema = z.coerce.number({ message: 'Must be a number' });
      if (validation.min !== undefined) {
        schema = (schema as z.ZodNumber).min(validation.min, `Must be at least ${validation.min}`);
      }
      if (validation.max !== undefined) {
        schema = (schema as z.ZodNumber).max(validation.max, `Must be at most ${validation.max}`);
      }
      break;
    ...
    case 'date':
    case 'datetime':
      schema = z.coerce.date({ message: 'Invalid date' });
      break;
...
export function createGridSchema(
  ...
  if (grid.validation.minRows > 0) {
    gridSchema = gridSchema.min(
      grid.validation.minRows,
      `At least ${grid.validation.minRows} rows required`
    );
  }
  if (grid.validation.maxRows !== undefined) {
    gridSchema = gridSchema.max(
      grid.validation.maxRows,
      `At most ${grid.validation.maxRows} rows allowed`
    );
  }
